...
ban = []
cr = '*MLSPACETOON*'
vz = '*THANKS 4 REGISTER MLSPACETOON*'
const vcard = 'BEGIN:VCARD\n' //
            + 'VERSION:3.0\n' //
            + 'FN: Fadhli\n' //
            + 'ORG:@itspapoy;\n' //
            + 'TEL;type=CELL;type=VOICE;waid=6287714745440:+6287714745440 \n' //
            + 'END:VCARD'
prefix = '.'
blocked = []
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('?','red'), color(']','white'), color(' Scan qode qr Muuu'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `(っ◔◡◔)っ𝗦𝗮𝘆𝗼𝗻𝗮𝗿𝗮𝗮𝗮👋🏻 @${num.split('@')[0]}\n𝘠𝘢𝘯𝘨 𝘬𝘦𝘭𝘶𝘢𝘳 𝘯𝘪𝘵𝘪𝘱 𝘴𝘦𝘣𝘭𝘢𝘲 𝘺𝘢𝘢 𝘮𝘸𝘩𝘩𝘦 ᥬ😉᭄`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				levelnol: '*LEVEL KAMU KOK MASIH * 0 >_<',
				error: {
				...
				only: {
					...
					premium: 'Maaf Brou Perintah Ini Hanya Untuk User Premium ',
					mod: 'Maaf kak Fitur Ini Untuk User Mods 𝙾𝚗𝚒𝚌𝚑𝚊𝚗',
					benned: 'Anda Ke Ban Silahkan Hubungi Owner Agar Membuka Ban Anda',
					ownerG: '[❗] Kamu siapa? ini fitur khusus owner njierrr ❌',
					ownerB: '[❗] Kamu siapa? ini fitur khusus owner njierrr ❌',
					userB: `「 PRA-PENDAFTARAN 」\nHallo Siss!\nKamu belum terdaftar di dalam database MLSPACETOON BOT \nCONTOH CARA DAFTAR nya Gini \n\n*${prefix}* daftar mlspacetoon|𝟷𝟽\n\n「 POWERED BY MLSPACETOON 」`,
			...
			const ownerNumber = ["6287714745440@s.whatsapp.net","628165466368@s.whatsapp.net"]
			const mod = [ownerNumber,"6287714745440@s.whatsapp.net","628165466368@s.whatsapp.net"]
			const adminbotnumber = ["6287714745440@s.whatsapp.net","628165466368@s.whatsapp.net"]
			const frendsowner = ["6287714745440@s.whatsapp.net","628165466368@s.whatsapp.net"]
			const premium = ["6287714745440@s.whatsapp.net","628165466368@s.whatsapp.net"]
			...
			switch(command) {
				...
				case 'menu':
			       ...
			       case 'valxlove':
					...
					break
					case 'mlspacetoonadmin':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					tod = await getBuffer(`https://i.ibb.co/XDwBVDJ/1f2652c622fa.jpg`)
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT MLSPACETOON ✨* 」\n*│+wa.me/6287714745440*╰──────*「 *MLSPACETOON* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  MLSPACETOON BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6287714745440*╰──────*「 * MLSPACETOON* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  MLSPACETOON BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
	...
	break
                ...
                case 'irii':
			client.sendPtt(from, './mlspacetoon/iri2.mp3', id)
			break
		case 'iri':
			client.sendPtt(from, './mlspacetoon/iri.mp3', {quoted: mek, ptt:true})
			break
                case 'baka':
            case 'onichan':
            case 'bodoh':
                client.sendMessage(dari, './mlspacetoon/baka.mp3', audio/mp3, {quoted: mek, ptt:true})
                break
			...
			case 'owner':
                 ...
                 client.sendMessage(from, 'Kenalin itu ownerq orangnya a6 sv aja ntar di svbck^_-',MessageType.text, { quoted: mek} )
                 client.sendMessage(from, 'Mutualan? instagram.com/itspapoy',MessageType.text, { quoted: mek} )
                 tod = await getBuffer(`https://i.ibb.co/XFrR3Qj/637cb6919692.jpg`)
                 client.sendMessage(from, tod, image, { quoted: mek, caption: '*Pararunten Bu Bidannnn!*'})
                 break
                 case 'emailtome':
            try {
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				case 'testime':
					...
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER BOT  MLSPACETOON👑* 」\n\`\`\``
					no = 0
 ...
 if (!isOwner) return reply(mess.only.ownerB)
            ...
            break
				...
				case 'ffbaner':
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/bannerff?title=${love}&text=MLSPACETOON&apikey=bli api biar work`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
					case 'goldbutton':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
		...
		if (args.length < 1) return reply('tag temanmu!')
                ...
                client.sendMessage(hasil, image, mek, '\`\`\`MLSPACETOON\`\`\`')
				break
					case 'kata':
          ...
          reply('Wait..')
            ...
            break
				...
				case 'kicktime':
					...
					setTimeout( () => {
					 client.sendMessage(from, 'siap siap ya stardust Kick nih', text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 10s,
					}
					break
				case 'aguse':
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (!isPremium) return reply(mess.only.premium)
			    ...
			    case 'otagall':
					...
					mentions('╔══✪ 〘 TAGALL 〙 ✪══'+teks+'╚═〘  MLSPACETOON BOT 〙', members_id, true)
					break
				case 'clearall':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * MLSPACETOON Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner MLSPACETOON!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin MLSPACETOON!')
				   }
                   break
				case 'wait':   
